...
> 更新：2026-08-20。v1.3（深化与债务清理）、v1.5（发布就绪）已全部完成：术语表旁车 / 自定义润色指令 / whisper.cpp 实验后端 / 大版本依赖迁移；安装包（Inno Setup + 文件关联）/ 自动更新（GitHub Releases 启动检查 + 一键升级）/ CI 流水线 / 崩溃与日志 / 发布文档（用户手册 + 隐私声明）。v2.0 自动化部分已落地：监视文件夹、系统托盘；多语言已按产品决策**回退为纯中文界面**（v2.0 发布前移除 l10n 体系）。v2.1.0：定时自动检查更新 + 3 项 functional 修复（队列清除失控子进程 / 退出链无保护 / MKVToolNix 严格解码）。本文档反映当前现状与后续路线。
>
> 当前质量基线（2026-08-20 复核）：`dart analyze` 零问题，`flutter test` 327/327。
