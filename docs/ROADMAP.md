# Subtitle Studio Pro —— 优化与更新计划

> 更新：2026-08-20。v1.3（深化与债务清理）、v1.5（发布就绪）已全部完成：术语表旁车 / 自定义润色指令 / whisper.cpp 实验后端 / 大版本依赖迁移；安装包（Inno Setup + 文件关联）/ 自动更新（GitHub Releases 启动检查 + 一键升级）/ CI 流水线 / 崩溃与日志 / 发布文档（用户手册 + 隐私声明）。v2.0 自动化部分已落地：监视文件夹、系统托盘；多语言已按产品决策**回退为纯中文界面**（v2.0 发布前移除 l10n 体系）。v2.1.0：定时自动检查更新 + 3 项 functional 修复（队列清除失控子进程 / 退出链无保护 / MKVToolNix 严格解码）。v2.1.1：minor 修复批（半成品输出清理 / 双开临时文件互踩 / 监视文件夹同名多视频）。本文档反映当前现状与后续路线。
>
> 当前质量基线（2026-08-20 复核）：`dart analyze` 零问题，`flutter test` 335/335。



## 五、v1.3 —— 深化与债务清理（约 1 个月）

| # | 项 | 说明 | 工作量 | 状态 |
|---|---|---|---|---|
| 1 | **术语表旁车文件化** | `.glossary.json` 跟随项目/目录，多任务共享词库；UI 导入导出 | 中 | ✅ 完成 |
| 2 | **自定义润色指令** | 设置页可编辑润色 system prompt 附加段（如「保留口语语气词」） | 小 | ✅ 完成 |
| 3 | **whisper.cpp 实验性后端** | v1.2 已交付评估结论；v1.3 做实验性枚举项（默认隐藏，检测到可执行文件才显示） | 中 | ✅ 完成（`-m/-f/-osrt/-of/-ng` 参数映射，ggml 模型名自动转换，预设/提示词不适用） |
| 5 | **大版本依赖迁移** | file_picker 8→12、share_plus 9→13、window_manager 0.3→0.5、riverpod 2→3（破坏性变更，逐个提交迁移，每步全量测试）；连带 desktop_drop 0.4→0.7、media_kit 1.1→1.2+（web 约束解绑后随迁） | 大 | ✅ 完成（file_picker 与 share_plus 因 win32 约束耦合同步迁移；全量测试 250/250） |

---

## 六、v1.5 —— 发布就绪（约 2~3 周）

| # | 项 | 说明 | 工作量 | 状态 |
|---|---|---|---|---|
| 1 | **安装包** | Inno Setup 或 MSIX：应用图标、开始菜单、文件关联（.srt/.ass）；可选捆绑 FFmpeg 便携版（解压即用） | 中 | ✅ 完成（Inno Setup 按用户安装；.srt/.ass/.ssa/.vtt 关联 + argv 启动播种字幕库；CI tag 构建产物） |
| 2 | **自动更新** | GitHub Releases + 启动检查 + 一键升级（差量可选，先做全量替换） | 中 | ✅ 完成（UpdateService + 启动静默检查 → 首页横幅；手动检查 / Release Notes / 下载进度 / /SILENT 静默升级，v2.1.0 起手动检查位于设置页「维护」分组，另有每 6 小时定时静默检查开关） |
| 3 | **CI 流水线** | GitHub Actions windows-latest：`flutter analyze` + `flutter test` + release 构建产物；PR 门禁（依赖 v1.2.x 的 git 仓库） | 中 | ✅ 完成（analyze + test + tag 触发 Inno Setup 构建发布；runner 装 MKVToolNix） |
| 4 | **崩溃与日志** | `FlutterError.onError` + 未捕获异常落盘；结构化日志（JSON）+ 轮转；设置页「导出调试包」一键打包日志与配置（脱敏 API Key） | 中 | ✅ 完成（CrashGuard + LogFileStore JSON Lines 轮转 + DebugBundle 脱敏导出） |
| 5 | **发布文档** | 用户手册（各功能页截图 + 常见问题）、隐私声明（哪些数据外发：仅 AI 翻译字幕文本） | 小 | ✅ 完成（docs/USER_MANUAL.md + docs/PRIVACY.md；截图待发布前补） |

---

## 七、v2.0 —— 自动化与生态（长期）

| # | 项 | 说明 | 工作量 | 状态 |
|---|---|---|---|---|
| 1 | **监视文件夹** | 指定目录 + 规则（如「新视频 + 同名字幕 → 自动烧录到输出目录」），无人值守流水线 | 大 | ✅ 完成（`WatchFolderService`：顶层配对 / 静默期稳定检测 / 存量不处理 / 输出已存在跨重启防重；设置页「自动化」开关 + 双目录配置，即时启停） |
| 2 | **系统托盘** | 最小化常驻，任务后台继续；托盘菜单：显示主窗 / 暂停队列 / 退出 | 中 | ✅ 完成（tray_manager；关闭最小化到托盘可关；队列暂停/恢复贯通主界面与托盘） |
| 3 | **多语言界面** | 中/英文案抽离（flutter ARB），面向海外字幕组 | 中 | ↩️ 已回退（v1.5 发布前按产品决策移除 l10n 体系：删除 ARB / 生成文件 / L10nHolder / 语言设置，界面回归纯中文；理由：目标用户以国内字幕组为主，维护双语文案成本高于收益。如需恢复可从 git 历史找回） |

---

## 七之二、v2.1.0 —— 更新体验与稳定性修复（2026-08-20）

| # | 项 | 说明 | 状态 |
|---|---|---|---|
| 1 | **定时自动检查更新** | 运行期间每 6 小时静默检查 GitHub Releases，发现新版本首页横幅提示；设置页「维护 → 自动检查更新」开关控制，默认开启 | ✅ 完成（`startPeriodicUpdateCheck` 注入缝可测，异常静默吞掉） |
| 2 | **检查更新迁入设置页** | 手动检查 / Release Notes / 下载进度 / 一键升级自关于页迁移至设置页「维护 → 版本与更新」，关于页回归纯信息展示 | ✅ 完成 |
| 3 | **队列清除失控子进程（functional）** | 「清空队列」/ 移除运行中任务时先取消 CancelToken 终止子进程，且被清除任务不写入历史 | ✅ 修复（含回归测试） |
| 4 | **退出链无保护（functional）** | `_exitApp` 每步 try/catch，托盘清理失败不阻断窗口销毁与应用退出 | ✅ 修复（`safeExitSequence`） |
| 5 | **MKVToolNix 严格 UTF-8 解码（functional）** | mkvmerge 输出解码统一改宽容模式（`allowMalformed`），GBK 文件名旧文件不再任务失败 | ✅ 修复 |
| 6 | **内嵌字幕烧录临时文件泄漏** | `burnEmbeddedTrack` 预提取 SRT 临时文件 try/finally 统一清理 | ✅ 修复（v2.0.1 批次，随本版发布） |
| 7 | **侧边栏品牌简化** | 移除侧边栏「SSP」字样，仅保留图标 | ✅ 完成 |

---

## 七之三、v2.1.1 —— minor 修复批（2026-08-20）

| # | 项 | 说明 | 状态 |
|---|---|---|---|
| 1 | **半成品输出清理** | 取消/失败的任务删除本任务运行期间写入的产物文件（mtime 快照守卫：任务前就存在且未触碰的旧文件保留；whisper 输出目录不误删） | ✅ 修复（含 6 条回归测试） |
| 2 | **双开实例临时文件互踩** | 内嵌轨烧录预提取临时文件 `embedded_track_<n>.srt` 固定名加 uuid 后缀，双实例同时烧同一轨道序号不再互删互写 | ✅ 修复 |
| 3 | **监视文件夹同名多视频静默猜配** | `movie.mp4` + `movie.mkv` + `movie.srt` 场景原依赖枚举顺序取后者；改为跳过配对并记 warn 日志，不猜 | ✅ 修复 |
| 4 | 排查报告误报甄别 | 翻译 checkpoint 取消保留是断点续传设计（不修）；监视输出目录清理涉及用户数据（不做）；whisper 静音探测临时文件名含时间戳（低风险不动） | ✅ 已甄别 |

---

## 八、质量门禁（每个版本必须满足）

1. **静态检查**：`dart analyze` 零问题（info 也不留）。
2. **测试全绿**：`flutter test` 全量通过；新增行为必须有先失败的测试（TDD）。
3. **布局回归**：UI 改动在 800×600 / 1024×700 / 1280×800 三档窗口无 RenderFlex 溢出。
4. **命名规则**：新产物命名一律走「内存 used 集合 + 磁盘双查」去重，禁止模板外硬编码。
5. **依赖检测**：与外部工具可用性相关的 UI 一律 `ValueListenableBuilder` 监听，禁止静态读取。
6. **性能基准**：烧录/转码以 1080p 24min 样片为基准集，改动前后对比耗时与产物体积，防回归。

---

## 九、已识别风险

| 风险 | 影响 | 对策 |
|---|---|---|
| ~~代码无版本控制~~（已解决） | 已不影响 | ✅ v1.2.x #1 完成：git 仓库已建立并推送 GitHub；v1.5 的 CI 可直接基于其落地 |
| openai-whisper 检测慢（torch 导入 10~30s） | 首次进入 Whisper 页体验差 | ✅ 已实现：检测结果持久化缓存 + 启动后台静默复检，指纹失效自动清缓存转不可用 |
| AI 翻译文本外发 | 隐私合规 | ✅ v1.5 已发布隐私声明（docs/PRIVACY.md）：外发范围（仅字幕文本 + 术语表 → 用户自配 API）、本地存储、调试包脱敏全部成文 |
| 串行队列长任务阻塞 | 批量大任务等待久 | v1.1 已网络/本地双车道并行；v2.0 托盘 + 通知缓解感知 |
| 停维护依赖（flutter_big5 / js） | 安全补丁缺失、未来 SDK 升级断供 | ✅ flutter_big5 已内置 vendor（`lib/services/subtitle/big5/`，BSD）；js 为间接依赖，随 SDK 升级观察 |
| Flutter SDK 含空格路径（开发机） | `flutter test`/`dart run` 间歇损坏 | 文档记录 workaround；CI 使用标准路径消除 |
